...
class Team(Base):
    ...
    # Contexte sportif (confort d'affichage) : n'entre dans AUCUN calcul de points.
    coach_name: Mapped[str | None] = mapped_column(String(150), nullable=True)
    coach_photo_url: Mapped[str | None] = mapped_column(String(255), nullable=True)
    # Identifiant API-Football mis en cache : évite de re-résoudre l'équipe par son nom à
    # chaque exécution du seed (économise le quota, 100 requêtes/jour en niveau gratuit).
    api_football_team_id: Mapped[int | None] = mapped_column(unique=True, nullable=True)
